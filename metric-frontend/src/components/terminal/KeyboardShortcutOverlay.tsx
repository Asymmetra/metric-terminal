...
export function KeyboardShortcutOverlay() {
  ...
  return (
    ...
    >
      <div className="w-[320px] border border-metric-border bg-surface-1 shadow-[0_16px_64px_rgba(0,0,0,0.6)]">
        {/* Header */}
        ...
        <div className="flex flex-col gap-0 p-2">
          {SHORTCUTS.map((s) => (
            <div
              key={s.key}
              className="flex items-center justify-between px-2 py-2 transition-colors hover:bg-surface-2/30"
            >
              <span className="font-mono text-[11px] text-text-secondary/70">{s.description}</span>
              <kbd className="min-w-[36px] border border-metric-border/50 bg-surface-2 px-2 py-0.5 text-center font-mono text-[10px] text-text-primary">
                {s.key}
              </kbd>
            </div>
          ))}
