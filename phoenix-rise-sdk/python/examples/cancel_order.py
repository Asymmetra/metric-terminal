...
Run with:
  ...
  python examples/cancel_order.py SOL 50000 12345
"""
...
from phoenix_sdk import CancelId, PhoenixHttpClient, PhoenixTxBuilder, TraderKey
...
async def main() -> None:
    if len(sys.argv) < 4:
        print("Usage: cancel_order.py <SYMBOL> <PRICE_IN_TICKS> <ORDER_SEQUENCE_NUMBER>")
        print("Example: cancel_order.py SOL 50000 12345")
        sys.exit(1)

    symbol = sys.argv[1]
    price_in_ticks = int(sys.argv[2])
    order_sequence_number = int(sys.argv[3])
    ...
    cancel_id = CancelId(price_in_ticks, order_sequence_number)
    print(f"\nCancelling order: {symbol} price_in_ticks={price_in_ticks} seq={order_sequence_number}")
    instructions = builder.build_cancel_orders(
        trader.authority, trader.pda(), symbol, [cancel_id],
    )
