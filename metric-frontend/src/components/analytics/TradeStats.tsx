...
function StatBox({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div className="flex flex-col gap-1 border border-metric-border bg-surface-1 p-3">
  ...
  return (
    <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
      ...
      <StatBox
        ...
        color={stats.winRate >= 0.5 ? "text-metric-buy" : "text-metric-sell"}
      />
      <StatBox label="Wins / Losses" value={`${stats.wins} / ${stats.losses}`} />
      <StatBox
        label="Total PnL"
        value={`${stats.totalPnl >= 0 ? "+" : ""}${formatUsd(stats.totalPnl)}`}
        color={stats.totalPnl >= 0 ? "text-metric-buy" : "text-metric-sell"}
      />
      <StatBox
        label="Net PnL"
        value={`${stats.netPnl >= 0 ? "+" : ""}${formatUsd(stats.netPnl)}`}
        color={stats.netPnl >= 0 ? "text-metric-buy" : "text-metric-sell"}
      />
      <StatBox label="Avg Win" value={`+${formatUsd(stats.avgWin)}`} color="text-metric-buy" />
      <StatBox label="Avg Loss" value={formatUsd(stats.avgLoss * -1)} color="text-metric-sell" />
      <StatBox label="Largest Win" value={`+${formatUsd(stats.largestWin)}`} color="text-metric-buy" />
      <StatBox label="Largest Loss" value={formatUsd(stats.largestLoss)} color="text-metric-sell" />
      <StatBox
        label="Profit Factor"
        value={stats.profitFactor === Infinity ? "∞" : stats.profitFactor.toFixed(2)}
        color={stats.profitFactor >= 1 ? "text-metric-buy" : "text-metric-sell"}
      />
      <StatBox
        label="Expectancy"
        value={`${stats.expectancy >= 0 ? "+" : ""}${formatUsd(stats.expectancy)}`}
        color={stats.expectancy >= 0 ? "text-metric-buy" : "text-metric-sell"}
      />
      <StatBox
        label="Sharpe Ratio"
        value={stats.sharpeRatio != null ? stats.sharpeRatio.toFixed(2) : "—"}
        color={stats.sharpeRatio != null && stats.sharpeRatio >= 1 ? "text-metric-buy" : stats.sharpeRatio != null && stats.sharpeRatio < 0 ? "text-metric-sell" : undefined}
      />
      <StatBox label="Win Streak" value={stats.maxConsecutiveWins.toString()} color="text-metric-buy" />
      <StatBox label="Loss Streak" value={stats.maxConsecutiveLosses.toString()} color="text-metric-sell" />
      <StatBox
        label="Fee/PnL Ratio"
        value={`${(stats.feeToPnlRatio * 100).toFixed(1)}%`}
        color={stats.feeToPnlRatio > 0.5 ? "text-metric-sell" : "text-text-primary"}
      />
