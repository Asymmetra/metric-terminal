    ...
    <div className="border border-ember-border bg-surface-l1">
      <div className="flex items-center justify-between border-b border-ember-border/60 px-4 py-2.5">
        <div className="flex items-center gap-2">
          <span className={clsx("font-mono text-xs font-medium", isIsolated ? "text-ember-orange" : "text-text-primary")}>
            {isIsolated ? `Isolated #${idx}` : "Cross Margin"}
          </span>
          <span className={clsx("px-1.5 py-0.5 font-mono text-[9px] uppercase tracking-wider", isIsolated ? "bg-ember-orange/10 text-ember-orange" : "bg-ember-green/10 text-ember-green")}>
            {isIsolated ? "isolated" : "cross"}
          </span>
        </div>
        <span className={clsx("font-mono text-[10px] uppercase tracking-wider", account.riskState === "Active" ? "text-ember-green" : account.riskState === "Warning" ? "text-yellow-500" : "text-text-secondary/40")}>
          {account.riskState || "—"}
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-px bg-ember-border/40 sm:grid-cols-3">
        ...
        <StatCell label="Unrealized PnL" value={`${pnl >= 0 ? "+" : ""}${formatUsd(pnl)}`} color={pnl >= 0 ? "text-ember-green" : "text-ember-red"} />
        <StatCell label="Margin Usage" value={`${marginUsage.toFixed(1)}%`} color={marginUsage > 80 ? "text-ember-red" : marginUsage > 50 ? "text-yellow-500" : "text-text-primary"} />
      ...
      {positionCount > 0 && (
        <div className="border-t border-ember-border/40">
          ...
          {(account.positions || []).map((pos: any, i: number) => {
            ...
            return (
              <div key={`${symbol}-${i}`} className="flex items-center justify-between border-t border-ember-border/20 px-4 py-1.5">
                <div className="flex items-center gap-2">
                  <span className="font-mono text-[11px] text-text-primary">{symbol}</span>
                  <span className={clsx("font-mono text-[10px]", side === "Long" ? "text-ember-green" : "text-ember-red")}>
                    {side} {Math.abs(size).toFixed(4)}
                  </span>
                </div>
                <div className="flex items-center gap-3">
                  <span className="font-mono text-[10px] text-text-secondary">@ {formatUsd(entry)}</span>
                  <span className={clsx("font-mono text-[10px] tabular-nums", upnl >= 0 ? "text-ember-green" : "text-ember-red")}>
...
function EmptySubaccountRow({ account }: { account: SubaccountData }) {
  ...
  return (
    <div className="flex items-center justify-between border border-ember-border/40 bg-surface-l1/50 px-4 py-2">
      <div className="flex items-center gap-2">
        <span className="font-mono text-[11px] text-ember-orange/60">Isolated #{idx}</span>
        <span className="px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-wider bg-ember-border/30 text-text-secondary/40">empty</span>
      </div>
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-ember-black">
        <div className="flex flex-col items-center gap-4">
          ...
          <Link href="/terminal" className="font-mono text-[11px] text-ember-orange hover:underline">Go to Terminal</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="flex min-h-screen flex-col bg-ember-black">
      ...
      <div className="flex items-center justify-between border-b border-ember-border bg-surface-l1 px-4 py-2">
        <div className="flex items-center gap-4">
          ...
          {isViewingOther && authority && (
            <>
              <div className="h-4 w-px bg-ember-border" />
              <span className="font-mono text-[10px] text-ember-orange">{truncateAddress(authority)}</span>
            </>
          )}
        </div>
        <div className="flex items-center gap-3">
          <Link href="/terminal" className="font-mono text-[10px] uppercase tracking-wider text-text-secondary/60 hover:text-ember-orange transition-colors">
            Terminal
          </Link>
          <Link href="/leaderboard" className="font-mono text-[10px] uppercase tracking-wider text-text-secondary/60 hover:text-ember-orange transition-colors">
            Leaderboard
          </Link>
          <WalletButton />
        </div>
      ...
      {!authority ? (
        ...
        <div className="flex flex-1 flex-col">
          {/* Tab bar */}
          <div className="flex border-b border-ember-border bg-surface-l1/50 px-4 overflow-x-auto">
            {TABS.map((tab) => (
              <button
                key={tab.key}
                ...
                className={clsx(
                  ...
                  activeTab === tab.key
                    ? "border-b-2 border-ember-orange text-text-primary"
