...
export function HistoryTabs({ authority, period }: Props) {
  ...
  return (
    <div className="border border-metric-border bg-surface-1">
      <div className="flex items-center border-b border-metric-border/60 overflow-x-auto">
        {TABS.map((t) => (
          <button
            key={t.key}
            ...
            className={clsx(
              ...
              tab === t.key
                ? "border-b border-metric-primary text-metric-primary"
...
function Th({ children, right }: { children: React.ReactNode; right?: boolean }) {
  return (
    <th
      className={clsx(
        "bg-surface-1 px-4 py-1.5 font-mono font-normal uppercase tracking-wider",
  ...
  return (
    <td
      className={clsx(
        ...
        colored === "green" && "text-metric-buy",
        colored === "red" && "text-metric-sell",
