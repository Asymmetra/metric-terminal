...
export function MultiOrderPanel() {
  ...
  return (
    <div className="flex flex-col gap-2">
      {/* Mode toggle */}
      <div className="flex border-b border-metric-border/30">
          ...
          <div className="grid grid-cols-2 gap-1.5">
            <div>
              ...
              <input
                ...
                className="w-full border border-metric-border bg-surface-2 py-1 px-2 font-mono text-[10px] text-text-primary placeholder:text-text-secondary/40 focus:outline-none focus:border-metric-primary/60"
              />
            </div>
            <div>
          ...
          <button
            ...
            className={clsx(
              ...
              gridParams.sizePerOrder && parseFloat(gridParams.sizePerOrder) > 0
                ? "bg-metric-primary/20 text-metric-primary border border-metric-primary/40 hover:bg-metric-primary/30"
                : "bg-surface-2 text-text-secondary/40 border border-metric-border cursor-not-allowed"
            )}
          >
            Generate Grid
          </button>
      ...
      {mode === "manual" && rows.length < MAX_ORDERS && (
        <button
          onClick={() => addRow()}
          className="w-full py-1 border border-dashed border-metric-border/50 text-[10px] text-text-secondary/50 hover:text-metric-primary hover:border-metric-primary/40 transition-colors"
        >
      ...
      {rows.length > 0 && (
        <button
          onClick={clearRows}
          className="w-full py-1 text-[9px] text-text-secondary/40 hover:text-metric-sell transition-colors"
        >
          Clear All
        </button>
      )}
        ...
        <div className="flex flex-col gap-1 border border-metric-border/30 bg-surface-2/30 p-2">
          <div className="flex justify-between">
            ...
            <span className="font-mono text-[10px] text-text-primary">
              {summary.bidCount > 0 && <span className="text-metric-buy">{summary.bidCount}B</span>}
              {summary.bidCount > 0 && summary.askCount > 0 && " / "}
              {summary.askCount > 0 && <span className="text-metric-sell">{summary.askCount}S</span>}
            </span>
          </div>
          <div className="flex justify-between">
      ...
      {connected && (
        <div className="flex items-center justify-between">
          ...
          <span className="font-mono text-[10px] text-metric-primary">{formatUsd(freeCollateral)}</span>
        </div>
      )}
    </div>
  );
    ...
    <div className="flex items-center gap-1">
      ...
      <button
        ...
        className={clsx(
          ...
          row.side === "bid"
            ? "bg-metric-buy/20 text-metric-buy border border-metric-buy/30"
            : "bg-metric-sell/20 text-metric-sell border border-metric-sell/30"
        )}
      >
        {row.side === "bid" ? "B" : "S"}
      </button>
      ...
      <input
        ...
        min="0"
        step="0.01"
        className="w-0 flex-1 border border-metric-border bg-surface-2 py-1 px-1.5 font-mono text-[10px] text-text-primary placeholder:text-text-secondary/30 focus:outline-none focus:border-metric-primary/60"
      />
      ...
      <input
        ...
        min="1"
        step="1"
        className="w-0 flex-[0.6] border border-metric-border bg-surface-2 py-1 px-1.5 font-mono text-[10px] text-text-primary placeholder:text-text-secondary/30 focus:outline-none focus:border-metric-primary/60"
      />

      {/* Remove button */}
      <button
        onClick={() => onRemove(row.id)}
        className="flex-shrink-0 w-5 h-5 flex items-center justify-center text-text-secondary/40 hover:text-metric-sell transition-colors"
      >
