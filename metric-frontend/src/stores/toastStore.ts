...
interface ToastStore {
  ...
  updateToast: (
    id: string,
    updates: Partial<Pick<Toast, "type" | "title" | "detail" | "txid">>
  ) => void;
...
export const useToastStore = create<ToastStore>((set) => ({
  ...
  addToast: (type, title, detail) => {
    ...
    set((s) => {
      ...
      // Max 3 visible — drop oldest non-loading first, else oldest overall.
      if (updated.length > 3) {
        const nonLoadingIdx = updated.findIndex((t) => t.type !== "loading");
        if (nonLoadingIdx >= 0) updated.splice(nonLoadingIdx, 1);
        else updated.shift();
      }
