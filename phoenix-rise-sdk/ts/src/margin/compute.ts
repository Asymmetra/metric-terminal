...
export const computeSubaccountMarginFromInputs = (
  ...
  const effectiveCollateral = collateralBalance + totalDiscountedUnrealizedPnl;
  const effectiveCollateralForWithdrawals =
    collateralBalance + totalDiscountedPnlForWithdrawals;
...
const marginIncreaseForBids = (
  ...
  const newExposure = newExposureSigned;
  const newGrossValue = assetUnitPrice * newExposure;
  ...
  if (bypassRiskFactor) {
    ...
    return maxBigInt(totalMargin - existingPositionMarginOffset, 0n);
  }

  const leverage = getLeverageConstant(tiers, newExposure);
  const leverageBasedMargin = divCeil(newGrossValue, leverage);
  const riskFactor = getLimitOrderRiskFactor(tiers, newExposure);
  return applyBpsCeil(leverageBasedMargin, riskFactor);
};
...
const computeLimitOrderMargins = (
  ...
  for (const order of orders) {
    ...
    const marginFactor = getLimitOrderRiskFactor(tiers, remainingBaseLots);
