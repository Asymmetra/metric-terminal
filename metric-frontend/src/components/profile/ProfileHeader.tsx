...
export function ProfileHeader({ authority, viewingOther, period, onPeriodChange }: Props) {
  ...
  return (
    <div className="border border-metric-border bg-surface-1">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-metric-border/60 px-4 py-2.5">
        <div className="flex items-center gap-2">
          <button
            onClick={copyAddress}
            title="Copy full address"
            className="group flex items-center gap-2 font-mono text-[11px] text-text-primary transition-colors hover:text-metric-primary"
          >
            <span>{truncateAddress(authority)}</span>
            <span className="font-mono text-[9px] uppercase tracking-wider text-text-secondary/40 group-hover:text-metric-primary">
              {copied ? "copied" : "copy"}
            </span>
          </button>
          {viewingOther && (
            <span className="border border-metric-primary/40 bg-metric-primary/10 px-1.5 py-0.5 font-mono text-[9px] uppercase tracking-wider text-metric-primary">
              viewing
            </span>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 px-4 py-4 sm:grid-cols-3">
        <BigStat
          ...
          subClass={upnlPositive ? "text-metric-buy" : "text-metric-sell"}
        />
        <BigStat
