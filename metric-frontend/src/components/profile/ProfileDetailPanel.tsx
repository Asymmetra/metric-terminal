      ...
      {open && detail && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 bottom-0 z-[90] w-full max-w-[380px] sm:w-[380px] border-l border-metric-border bg-surface-1 shadow-[-16px_0_64px_rgba(0,0,0,0.4)] overflow-y-auto"
          >
  ...
  return (
    <>
      ...
      <DetailRow label="Side">
        <span className={buy ? "text-metric-buy" : "text-metric-sell"}>
          {buy ? "BUY" : "SELL"}
        </span>
      </DetailRow>
      <DetailRow label="Type">
        <span className={clsx(
          ...
          t.tradeType === "liquidation"
            ? "bg-metric-sell/15 text-metric-sell"
      ...
      <DetailRow label="Realized PnL">
        {t.realizedPnl === 0 ? (
          ...
          <span className={t.realizedPnl > 0 ? "text-metric-buy" : "text-metric-sell"}>
      ...
      <DetailRow label="Net">
        {net === 0 ? (
          ...
          <span className={net > 0 ? "text-metric-buy" : "text-metric-sell"}>
      ...
      <DetailRow label="Status">
        <span className={clsx(
          ...
          status.toLowerCase().includes("fill")
            ? "bg-metric-buy/15 text-metric-buy"
            : status.toLowerCase().includes("cancel")
              ? "bg-surface-2 text-text-secondary/60"
  ...
  return (
    <>
      ...
      <DetailRow label="Position">
        <span className={long ? "text-metric-buy" : short ? "text-metric-sell" : undefined}>
      ...
      <DetailRow label="Amount">
        {payment === 0 ? (
          ...
          <span className="text-metric-buy">+{formatUsdPrecise(Math.abs(payment))}</span>
        ) : (
          <span className="text-metric-sell">-{formatUsdPrecise(payment)}</span>
        )}
      </DetailRow>
...
function CollateralDetail({ c }: { c: Record<string, unknown> }) {
  ...
  return (
    <>
      ...
      <DetailRow label="Amount">
        <span className={amount > 0 ? "text-metric-buy" : amount < 0 ? "text-metric-sell" : ""}>
...
function PerMarketDetail({ r, period }: { r: PerMarketStats; period: Period }) {
  ...
  return (
    <>
      ...
      <SectionHeader title="PnL" />
      <DetailRow label="Realized PnL">
        <span className={pnlPos ? "text-metric-buy" : "text-metric-sell"}>
          {pnlPos ? "+" : ""}{formatUsdPrecise(r.realizedPnl)}
        </span>
      </DetailRow>
      <DetailRow label="Fees">{formatUsdPrecise(r.fees)}</DetailRow>
      <DetailRow label="Net">
        <span className={netPos ? "text-metric-buy" : "text-metric-sell"}>
  ...
  return (
    <>
      ...
      <DetailRow label="Unrealized PnL">
        <span className={upnlPositive ? "text-metric-buy" : "text-metric-sell"}>
      ...
      <DetailRow label="Liquidation Price">
        {p.liqPrice != null ? (
          <span className="text-metric-sell">${formatPrice(p.liqPrice)}</span>
        ) : (
          <span className="text-text-secondary/40">—</span>
        )}
      </DetailRow>
      ...
      <DetailRow label="Take Profit">
        {p.tp != null ? (
          <span className="text-metric-buy">${formatPrice(p.tp)}</span>
        ) : (
          <span className="text-text-secondary/40">—</span>
        )}
      </DetailRow>
      <DetailRow label="Stop Loss">
        {p.sl != null ? (
          <span className="text-metric-sell">${formatPrice(p.sl)}</span>
        ) : (
          <span className="text-text-secondary/40">—</span>
        )}
      </DetailRow>
