...
export function ConnectionPanel({ connection, onExportCsv, onResetStats, enableAllMids, onToggleAllMids }: Props) {
  const statusColor =
    connection.state === "connected" ? "text-metric-buy"
    : connection.state === "reconnecting" || connection.state === "connecting" ? "text-yellow-500"
    : "text-metric-sell";
  const dot =
    connection.state === "connected" ? "bg-metric-buy"
  ...
  return (
    <div className="border border-metric-border bg-surface-1">
      <div className="flex items-center gap-6 border-b border-metric-border/60 px-4 py-2.5">
        ...
        <div className="ml-auto flex items-center gap-3">
          <label className="flex items-center gap-1.5 cursor-pointer">
            <input
              ...
              className="accent-metric-primary"
            />
            <span
          ...
          <button
            onClick={onExportCsv}
            className="border border-metric-primary/40 bg-metric-primary/10 px-2 py-1 font-mono text-[10px] uppercase tracking-wider text-metric-primary hover:bg-metric-primary/20 transition-colors"
          >
            Export CSV
          </button>
        </div>
