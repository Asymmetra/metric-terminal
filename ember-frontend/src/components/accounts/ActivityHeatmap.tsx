...
export function ActivityHeatmap({ authority }: ActivityHeatmapProps) {
  ...
  function getCellColor(data: DayData | null): string {
    if (!data) return "bg-surface-l2/20";
    if (mode === "activity") {
      if (data.tradeCount === 0) return "bg-surface-l2/30";
      const intensity = data.tradeCount / maxCount;
      if (intensity > 0.75) return "bg-ember-green";
      if (intensity > 0.5) return "bg-ember-green/60";
      if (intensity > 0.25) return "bg-ember-green/35";
      return "bg-ember-green/15";
    }
    if (data.pnl === 0) return "bg-surface-l2/30";
    const intensity = Math.min(Math.abs(data.pnl) / maxAbsPnl, 1);
    if (data.pnl > 0) {
      return intensity > 0.5 ? "bg-ember-green" : "bg-ember-green/40";
    }
    return intensity > 0.5 ? "bg-ember-red" : "bg-ember-red/40";
  }
  ...
  return (
    <div className="border border-ember-border bg-surface-l1 p-4">
      ...
      {tooltip && (
        <div
          className="fixed z-50 border border-ember-border bg-surface-l2 px-2 py-1 shadow-lg pointer-events-none"
        ...
        >
          ...
          <div className={clsx("font-mono text-[9px]", tooltip.data.pnl >= 0 ? "text-ember-green" : "text-ember-red")}>
