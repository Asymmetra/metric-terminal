...
function ActionButton({ onClick, title, children }: { onClick?: () => void; title: string; children: React.ReactNode }) {
  return (
    <button
      onClick={onClick}
      className="shrink-0 p-1 text-white/50 transition-colors hover:bg-white/10 hover:text-white"
      title={title}
    >
      {children}
    </button>
...
function ToastItem({ toast }: { toast: Toast }) {
  ...
  const scheduleDismiss = useCallback(() => {
    ...
    timerRef.current = setTimeout(() => {
      if (!hoveredRef.current) removeToast(toast.id);
    }, DISMISS_MS);
  }, [toast.id, toast.type, removeToast]);

  useEffect(() => {
    if (toast.type !== "loading") scheduleDismiss();
    return () => {
  ...
  const bgColor = {
    success: "bg-[#0B1F22] border-metric-buy/30 border-l-metric-buy",
    error: "bg-[#1F1206] border-metric-sell/30 border-l-metric-sell",
    info: "bg-surface-2 border-metric-border border-l-metric-primary",
    loading: "bg-surface-2 border-metric-border border-l-metric-primary",
  }[toast.type];

  const textColor = {
    success: "text-metric-buy",
    error: "text-metric-sell",
    info: "text-text-secondary",
    loading: "text-metric-primary",
  }[toast.type];
  ...
  return (
    <div
      ...
      className={clsx(
        "toast-slide-in flex items-start gap-3 border border-l-[3px] px-4 py-3 font-mono",
        ...
        {toast.txid && (
          <a
            ...
            className={clsx(
              ...
              toast.type === "success" ? "text-metric-buy/70" : "text-metric-primary/70"
            )}
          >
...
export function Toasts() {
  ...
  return (
    <div className="fixed bottom-3 right-3 z-[200] flex w-[440px] flex-col-reverse gap-2">
      {toasts.map((toast) => (
