    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-metric-bg">
        <div className="flex flex-col items-center gap-4">
          ...
          <Link
            href="/terminal"
            className="font-mono text-[11px] text-metric-primary hover:underline"
          >
            Go to Terminal
          </Link>
        </div>
    ...
    <div className="flex min-h-screen flex-col bg-metric-bg">
      ...
      <div className="flex items-center justify-between border-b border-metric-border bg-surface-1 px-4 py-2">
        ...
        <div className="flex items-center gap-3">
          <Link
            href="/terminal"
            className="font-mono text-[10px] uppercase tracking-wider text-text-secondary/60 hover:text-metric-primary transition-colors"
          >
            Terminal
          </Link>
          <Link
      ...
      <div className="flex flex-1 flex-col gap-4 p-4">
            ...
            {PERIODS.map((p) => (
              <button
                ...
                className={clsx(
                  ...
                  period === p.key
                    ? "bg-metric-primary/10 text-metric-primary border border-metric-primary/40"
                    : "text-text-secondary/60 border border-metric-border hover:text-text-secondary"
                )}
              >
                {p.label}
              </button>
        ...
        {loading ? (
          ...
          <div className="overflow-hidden border border-metric-border">
            <table className="w-full">
              <thead>
                <tr className="border-b border-metric-border bg-surface-1">
              ...
              <tbody>
                {entries.map((entry, i) => (
                  <tr
                    key={entry.authority}
                    className="border-b border-metric-border/40 last:border-b-0 transition-colors hover:bg-surface-1/50 cursor-pointer"
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5">
                      <span className="font-mono text-xs text-text-primary hover:text-metric-primary transition-colors">
                      ...
                      return (
                        <td
                          className={clsx(
                            ...
                            periodPnl >= 0 ? "text-metric-buy" : "text-metric-sell"
                          )}
                        >
                          {periodPnl >= 0 ? "+" : ""}
                          {formatUsd(periodPnl)}
                    ...
                    <td
                      className={clsx(
                        "px-3 py-2.5 text-right font-mono text-xs tabular-nums",
                        entry.cumulative_pnl >= 0 ? "text-metric-buy" : "text-metric-sell"
                      )}
                    >
