  ...
  return (
    <div className="border border-metric-border bg-surface-1 p-4">
        ...
        {buckets.map((b, i) => {
          ...
          return (
            <div key={i} className="group relative flex flex-1 flex-col items-center">
              ...
              {/* Tooltip */}
              <div className="pointer-events-none absolute -top-12 left-1/2 z-50 hidden -translate-x-1/2 whitespace-nowrap border border-metric-border bg-surface-2 px-2 py-1 shadow-lg group-hover:block">
      ...
      <div className="mt-1 flex justify-between">
        <span className="font-mono text-[8px] text-metric-sell/60">
          {formatUsd(buckets[0]?.min || 0)}
        </span>
        <span className="font-mono text-[8px] text-text-secondary/40">$0</span>
        <span className="font-mono text-[8px] text-metric-buy/60">
