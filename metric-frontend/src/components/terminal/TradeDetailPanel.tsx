...
function DetailRow({
  ...
  return (
    <div className="flex items-center justify-between py-1.5">
      ...
      {hasTip && show && (
        <div
          className="fixed z-[200] w-60 rounded border border-metric-border bg-[#1A1B20] px-3 py-2.5 text-left text-[10px] normal-case tracking-normal text-text-secondary/90 leading-relaxed shadow-[0_8px_32px_rgba(0,0,0,0.6)] pointer-events-none"
          style={{
...
function SectionHeader({ title }: { title: React.ReactNode }) {
  return (
    <div className="pt-3 pb-1 mt-2 border-t border-metric-border/30">
      ...
      {open && detail && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 bottom-0 z-[90] w-[380px] border-l border-metric-border bg-surface-1 shadow-[−16px_0_64px_rgba(0,0,0,0.4)] overflow-y-auto"
          >
            {/* Header */}
  ...
  return (
    <>
      ...
      <DetailRow label="Side" tooltipTitle="Side" tooltip="Position direction. LONG profits when price rises; SHORT profits when price falls.">
        <span className={isLong ? "text-metric-buy" : "text-metric-sell"}>
      ...
      >
        <span className={pnlPct >= 0 ? "text-metric-buy" : "text-metric-sell"}>
      ...
      >
        <span className={liveUnrealizedPnl >= 0 ? "text-metric-buy" : "text-metric-sell"}>
      ...
      >
        <span className={data.discounted_unrealized_pnl >= 0 ? "text-metric-buy" : "text-metric-sell"}>
      ...
      >
        <span className={roi >= 0 ? "text-metric-buy" : "text-metric-sell"}>
          {roi >= 0 ? "+" : ""}{roi.toFixed(2)}%
        </span>
      </DetailRow>
      {Math.abs(live.unsettledFunding) > 0.0001 && (
        ...
        >
          <span className={live.unsettledFunding >= 0 ? "text-metric-buy" : "text-metric-sell"}>
      ...
      {Math.abs(live.accumulatedFunding) > 0.0001 && (
        ...
        >
          <span className={live.accumulatedFunding >= 0 ? "text-metric-buy" : "text-metric-sell"}>
      ...
      >
        {data.liquidation_price != null
          ? <span className="text-metric-sell">${formatPrice(data.liquidation_price)}</span>
      ...
      >
        {liqDistPct != null
          ? <span className={liqDistPct < 5 ? "text-metric-sell" : liqDistPct < 10 ? "text-yellow-500" : "text-metric-buy"}>
      ...
      >
        <span className="inline-flex items-center gap-2">
          {data.tp_price != null ? (
            <>
              <span className="text-metric-buy">${formatPrice(data.tp_price)}</span>
              <button
                ...
                className={clsx(
                  ...
                  cancellingLeg !== null
                    ...
                    : "text-metric-sell/60 hover:text-metric-sell"
                )}
              >
      ...
      >
        <span className="inline-flex items-center gap-2">
          {data.sl_price != null ? (
            <>
              <span className="text-metric-sell">${formatPrice(data.sl_price)}</span>
              <button
      ...
      {data.sl_price != null && (
        ...
        >
          <span className="text-metric-sell">
    ...
    <>
      ...
      <DetailRow label="Transaction" tooltipTitle="Transaction Signature" tooltip="The Solana transaction that contains this fill. Click to view the full instruction trace on Orb.">
        <a
          ...
          className="inline-flex items-center gap-1 text-metric-primary hover:underline"
        >
  ...
  return (
    <>
      <DetailRow label="Side" tooltipTitle="Side" tooltip="Whether the taker side of this print was a buy (bid) or a sell (ask). The opposite side was the maker on the book.">
        <span className={isBuy ? "text-metric-buy" : "text-metric-sell"}>
          {isBuy ? "BUY" : "SELL"}
        </span>
      </DetailRow>
