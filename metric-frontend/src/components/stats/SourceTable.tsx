...
const CATEGORY_LABELS: Record<SourceCategory, string> = {
  ...
  "metric-ws":     "Metric Backend · WebSocket",
  "metric-rest":   "Metric Backend · REST",
};
...
const CATEGORY_DESC: Record<SourceCategory, string> = {
  ...
  "metric-ws":     "Our backend's relayed streams (wss://ember-backend-q4nf.onrender.com/ws). Same data as Phoenix WS but routed through the metric relay.",
  "metric-rest":   "Our backend's HTTP API (ember-backend-q4nf.onrender.com).",
};
...
export function SourceTable({ sources, expanded, onToggle, selectedId, onSelect, filter, hiddenKinds }: Props) {
  ...
  const categoryOrder: SourceCategory[] = ["phoenix-ws", "phoenix-rest", "metric-ws", "metric-rest"];
  ...
  return (
    <div className="flex flex-col gap-3">
      {categoryOrder.map((cat) => {
        ...
        return (
          <div key={cat} className="border border-metric-border bg-surface-1">
            <button
              onClick={() => onToggle(cat)}
              className="flex w-full items-center justify-between border-b border-metric-border/60 px-4 py-2 text-left hover:bg-surface-2/50 transition-colors"
            >
              ...
              <div className="flex items-center gap-3 font-mono text-[10px]">
                <span className="text-metric-buy">● {healthy}</span>
                <span className="text-yellow-500">● {degraded}</span>
                <span className="text-metric-sell">● {stale}</span>
                    ...
                    {displayRows.map((row) => {
                      ...
                      return (
                        <tr
                          ...
                          className={clsx(
                            "cursor-pointer border-b border-metric-border/20 transition-colors",
                            selected ? "bg-metric-primary/10" : "hover:bg-surface-2/30",
                          )}
                        >
                          <td className="px-3 py-1.5 overflow-hidden">
                            <div className="flex items-center gap-2 truncate">
                              ...
                              {row.channelBadge && (
                                <span className="shrink-0 rounded border border-metric-border/50 bg-surface-2/60 px-1 py-0.5 text-[8px] uppercase tracking-wider text-text-secondary/70">
                                  {row.channelBadge}
                                </span>
                              )}
                            </div>
                          ...
                          <td className="overflow-hidden px-3 py-1.5 text-right font-mono text-[10px]">
                            {row.spread ? (
                              <BpsValue
                                ...
                                className={Math.abs(row.spread.markOracleBps) < 1 ? "text-text-secondary/60" : row.spread.markOracleBps > 0 ? "text-metric-buy" : "text-metric-sell"}
                              />
                            ) : (
      ...
      {title ? (
        <span className="group relative inline-block cursor-help border-b border-dotted border-text-secondary/30">
          ...
          <span
            role="tooltip"
            className={clsx(
              "pointer-events-none invisible absolute top-full z-50 mt-1 normal-case whitespace-normal rounded border border-metric-border bg-surface-l3/95 px-2 py-1.5 font-mono text-[10px] leading-snug tracking-normal text-text-primary opacity-0 shadow-xl backdrop-blur-sm transition-opacity duration-75 group-hover:visible group-hover:opacity-100",
