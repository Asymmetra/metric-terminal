...
function SubaccountCard({ account }: { account: Subaccount }) {
  ...
  return (
    <div className="bg-surface-1">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-metric-border/40 px-4 py-2.5">
        <div className="flex items-center gap-3">
          <span
            className={clsx(
              ...
              account.isolated
                ...
                : "border-metric-border bg-surface-2 text-text-secondary/60"
            )}
          >
          ...
          <span
            className={clsx(
              "font-mono text-[10px]",
              upnlPositive ? "text-metric-buy" : "text-metric-sell"
            )}
          >
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5">
            ...
            <div className="h-1 w-20 bg-metric-border/40">
              <div
                className={clsx("h-full", risky ? "bg-metric-sell" : "bg-metric-buy")}
            ...
            <span
              className={clsx(
                ...
                risky ? "text-metric-sell" : "text-text-secondary"
              )}
            >
              ...
              {account.positions.map((p) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className={clsx("px-4 py-1.5 font-medium", isLong ? "text-metric-buy" : "text-metric-sell")}>
                      {isLong ? "LONG" : "SHORT"}
                    </td>
                    ...
                    <td className={clsx("px-4 py-1.5 text-right tabular-nums", upnl >= 0 ? "text-metric-buy" : "text-metric-sell")}>
                      {upnl >= 0 ? "+" : ""}{formatUsd(upnl)}
                    </td>
                    <td className="px-4 py-1.5 text-right tabular-nums text-metric-sell/80">
                      {p.liqPrice != null ? `$${formatPrice(p.liqPrice)}` : "—"}
                    </td>
                    <td className="px-4 py-1.5 text-right tabular-nums text-metric-buy/80">
...
function RiskPill({ state }: { state: string }) {
  ...
  const color = healthy
    ? "border-metric-buy/40 bg-metric-buy/10 text-metric-buy"
    : "border-metric-sell/40 bg-metric-sell/10 text-metric-sell";
  return (
