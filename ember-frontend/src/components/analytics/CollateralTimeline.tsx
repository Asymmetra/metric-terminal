    ...
    <div className="flex flex-col gap-0 overflow-y-auto">
      {events.map((ev, i) => {
        ...
        return (
          <div
            key={`${ev.timestamp}-${i}`}
            className="flex items-center gap-3 border-b border-ember-border/40 px-4 py-2.5 last:border-b-0"
          >
            {/* Dot indicator */}
            <div
              className={clsx(
                "h-1.5 w-1.5 shrink-0",
                isDeposit ? "bg-ember-green" : "bg-ember-red"
              )}
            />
            ...
            <span
              className={clsx(
                ...
                isDeposit ? "text-ember-green" : "text-ember-red"
              )}
            >
              {isDeposit ? "+" : "-"}
