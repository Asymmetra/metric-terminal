...
function HealthBadge({ health }: { health: SymbolStats["health"] }) {
  const styles: Record<SymbolStats["health"], { label: string; cls: string; dot: string }> = {
    healthy:   { label: "Healthy",  cls: "text-metric-buy",        dot: "bg-metric-buy" },
    degraded:  { label: "Degraded", cls: "text-yellow-500",         dot: "bg-yellow-500" },
    stale:     { label: "Stale",    cls: "text-metric-sell",          dot: "bg-metric-sell" },
...
export function OracleStatsTable({ rows }: Props) {
  ...
  return (
    <div className="overflow-x-auto border border-metric-border bg-surface-1">
      <table className="w-full min-w-[1000px] font-mono text-[11px]">
        <thead>
          <tr className="border-b border-metric-border/60 text-[10px] text-text-secondary/60">
        ...
        <tbody>
          {sorted.map((r) => (
            <tr key={r.symbol} className="border-b border-metric-border/30">
              <td className="px-3 py-1.5 text-text-primary">{r.symbol}</td>
              <td className="px-3 py-1.5 text-right text-metric-primary">
              ...
              <td className={clsx(
                ...
                r.fundingRate != null && r.fundingRate >= 0 ? "text-metric-buy" : "text-metric-sell",
              )}>
