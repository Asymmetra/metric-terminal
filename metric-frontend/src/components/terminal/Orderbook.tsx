  ...
  return (
    <div
      ...
      className={clsx(
        ...
        isHovered ? "bg-surface-2/80" : "hover:bg-surface-2/60"
      )}
      style={{ height: "22px" }}
    >
      {/* Flash overlay */}
      <div
        ...
        style={{ "--flash-color": isBid ? "rgba(34,211,238,0.12)" : "rgba(249,115,22,0.12)" } as React.CSSProperties}
      />
      ...
      <div
        className={clsx(
          ...
          isBid ? "right-0 bg-metric-buy/[0.12]" : "right-0 bg-metric-sell/[0.12]"
        )}
      ...
      {isInHoverRange && !isHovered && (
        <div className={clsx(
          ...
          isBid ? "bg-metric-buy/[0.06]" : "bg-metric-sell/[0.06]"
        )} />
      )}

      {/* Row data */}
      <div className="relative grid w-full grid-cols-3 font-mono text-[11px] leading-none">
        <span className={isBid ? "text-metric-buy" : "text-metric-sell"}>
    ...
    <div ref={containerRef} className="relative flex h-full flex-col overflow-hidden">
      ...
      <div className="flex items-center px-2 py-1 text-[10px] text-text-secondary/70">
        ...
        <select
          ...
          className="ml-2 bg-surface-2 border border-metric-border/50 px-1 py-0.5 font-mono text-[9px] text-text-secondary/70 focus:outline-none focus:border-metric-primary/40 cursor-pointer"
        >
      ...
      {/* Spread bar */}
      <div className="flex items-center justify-between border-y border-metric-border/50 bg-metric-bg/50 px-2" style={{ height: "24px" }}>
      ...
      {depthStats && (
        <div
          className="grid grid-cols-[auto_1fr_1fr] items-center gap-x-2 border-t border-metric-border/50 bg-metric-bg/50 px-2 py-1 font-mono text-[9px]"
        >
          <span className="text-text-secondary/50">1%</span>
          <span className="text-right text-metric-buy">${abbreviateNumber(depthStats.bid1)}</span>
          <span className="text-right text-metric-sell">${abbreviateNumber(depthStats.ask1)}</span>
          <span className="text-text-secondary/50">2%</span>
          <span className="text-right text-metric-buy">${abbreviateNumber(depthStats.bid2)}</span>
          <span className="text-right text-metric-sell">${abbreviateNumber(depthStats.ask2)}</span>
        </div>
      )}
      ...
      {hoverInfo && hoverStats && tooltipPosition && (
        <div
          className="fixed z-[200] w-[230px] border border-metric-border bg-[#1A1B20] p-2.5 shadow-[0_8px_32px_rgba(0,0,0,0.6)] pointer-events-none"
          style={{
          ...
          <div className="flex flex-col gap-1.5">
            <div className="flex items-center justify-between">
              ...
              <span className={clsx(
                ...
                hoverInfo.side === "bid" ? "text-metric-buy" : "text-metric-sell"
              )}>
                {hoverInfo.side === "bid" ? "BID" : "ASK"}
              </span>
            </div>

            <div className="h-px bg-metric-border/50" />
            ...
            <div className="flex justify-between">
              ...
              <span className={clsx(
                "font-mono text-[10px]",
                hoverStats.distFromMid < 1 ? "text-metric-buy" : hoverStats.distFromMid < 2 ? "text-yellow-500" : "text-metric-sell"
              )}>
            ...
            <div className="flex justify-between">
              ...
              <span className={clsx(
                "font-mono text-[10px] font-medium",
                hoverStats.slippage < 0.1 ? "text-metric-buy" : hoverStats.slippage < 0.5 ? "text-yellow-500" : "text-metric-sell"
              )}>
                {hoverStats.slippage.toFixed(3)}%
              </span>
            </div>

            <div className="h-px bg-metric-border/30" />
            ...
            <div className="flex items-center gap-1.5">
              <span className={clsx(
                ...
                hoverStats.distFromMid < 1 ? "bg-metric-buy" : hoverStats.distFromMid < 2 ? "bg-yellow-500" : "bg-metric-sell"
              )} />
