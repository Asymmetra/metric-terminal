...
export function CodeBlock({ snippets, defaultLanguage, onLanguageChange }: Props) {
  ...
  return (
    <div className="border border-metric-border bg-surface-2/40">
      <div className="flex items-center border-b border-metric-border/50">
        {snippets.map((s) => (
          <button
            key={s.language}
            onClick={() => {
            ...
            className={clsx(
              "px-3 py-1.5 font-mono text-[10px] uppercase tracking-wider transition-colors border-r border-metric-border/40",
        ...
        <button
          onClick={handleCopy}
          className="ml-auto px-3 py-1.5 font-mono text-[10px] uppercase tracking-wider text-text-secondary/60 hover:text-metric-primary transition-colors"
        >
          {copied ? "Copied!" : "Copy"}
        </button>
      </div>
