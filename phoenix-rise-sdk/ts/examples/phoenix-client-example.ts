...
async function main() {
  ...
  if (!args[0]) {
    console.log("ℹ No trader pubkey provided. Using dummy pubkey for trader queries.");
  ...
  console.log(`  API client: ${client.api ? "initialized" : "not initialized"}`);
  ...
  try {
    ...
    try {
      const trader = await client.api.traders().getTraderState(traderPubkey);
      console.log("✓ Trader state fetched");
      console.log(`  Positions: ${JSON.stringify(trader).substring(0, 80)}...`);
    } catch (error) {
      console.log(`⚠ Could not fetch trader state: ${(error as Error).message}`);
    }

    // Example 7: Get order history
    console.log("\n=== Example 7: Get trader order history ===");
    try {
      const orders = await client.api.orders().getTraderOrderHistory(traderPubkey, {
        limit: 10,
      });
    ...
    try {
      const trades = await client.api.trades().getTraderTradesHistory(traderPubkey, {
        marketSymbol: "SOL",
        limit: 2,
      });
    ...
    try {
      ...
      console.log(`⚠ Could not fetch collateral history: ${(error as Error).message}`);
    }

    // Example 10: Get funding history
    console.log("\n=== Example 10: Get trader funding history ===");
    try {
      const funding = await client.api.funding().getTraderFundingHistory(traderPubkey, {
        symbol: "SOL",
        limit: 10,
      });
      ...
      console.log(`⚠ Could not fetch funding history: ${(error as Error).message}`);
    }

    // Example 11: Get funding rate history (market data)
    console.log("\n=== Example 11: Get funding rate history (SOL) ===");
    try {
      const fundingRates = await client.api.funding().getFundingRateHistory("SOL", {
        limit: 10,
      });
      console.log("✓ Funding rate history fetched");
      console.log(`  Data points: ${fundingRates.rates?.length ?? "N/A"}`);
    } catch (error) {
      console.log(`⚠ Could not fetch funding rates: ${(error as Error).message}`);
    }
    ...
    try {
      ...
      console.log(`⚠ Could not fetch market fills: ${(error as Error).message}`);
    }
