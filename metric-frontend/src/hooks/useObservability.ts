 ...
 *  - One Metric backend WebSocket (wss://ember-backend-q4nf.onrender.com/ws)
  ...
  switch (kind) {
    ...
    case "metric-rest-markets":
      return {
        ...
        endpoint: `${METRIC_REST_URL}/api/markets`,
        expectedCadenceMs: 30_000,
      };
    case "metric-rest-health-memory":
      return {
        ...
        endpoint: `${METRIC_REST_URL}/health/memory`,
        expectedCadenceMs: 30_000,
      };
    case "metric-rest-health-relay":
      return {
        ...
        endpoint: `${METRIC_REST_URL}/health/relay`,
        expectedCadenceMs: 30_000,
      };
    case "metric-rest-health-ws":
      return {
        ...
        endpoint: `${METRIC_REST_URL}/health/ws`,
        expectedCadenceMs: 30_000,
      };
    default:
      return {
...
export function useObservability(options: UseObservabilityOptions) {
  ...
  const [snapshot, setSnapshot] = useState<ObservabilitySnapshot>(() => ({
    ...
    categoryHealth: {
      ...
      "metric-ws": { healthy: 0, degraded: 0, stale: 0, error: 0, idle: 0, total: 0 },
      "metric-rest": { healthy: 0, degraded: 0, stale: 0, error: 0, idle: 0, total: 0 },
    },
  ...
  useEffect(() => {
    const pollers: Array<{ kind: SourceKind; url: string; intervalMs: number }> = [
      { kind: "metric-rest-markets",       url: `${METRIC_REST_URL}/api/markets`,    intervalMs: 30_000 },
      { kind: "metric-rest-health-memory", url: `${METRIC_REST_URL}/health/memory`,  intervalMs: 30_000 },
      { kind: "metric-rest-health-relay",  url: `${METRIC_REST_URL}/health/relay`,   intervalMs: 30_000 },
      { kind: "metric-rest-health-ws",     url: `${METRIC_REST_URL}/health/ws`,      intervalMs: 30_000 },
    ];
