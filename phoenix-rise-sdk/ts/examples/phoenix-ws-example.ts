...
Usage: bun run ts/examples/phoenix-ws-example.ts [CHANNEL] [ARGS...] [CHANNEL] [ARGS...] ...
...
Examples:
  bun run ts/examples/phoenix-ws-example.ts allMids
  bun run ts/examples/phoenix-ws-example.ts market SOL exchangeStatus
  bun run ts/examples/phoenix-ws-example.ts candles SOL 1m fundingRate SOL market BTC
  bun run ts/examples/phoenix-ws-example.ts traderState Drm7CaM8rniCWAgSkULcVAyXketYByjwjWSMhVHmPvdD 0
  bun run ts/examples/phoenix-ws-example.ts notifications Drm7CaM8rniCWAgSkULcVAyXketYByjwjWSMhVHmPvdD
`);
};
...
const main = async () => {
  ...
  const wsClient = createPhoenixWsClient({
    url: "wss://perp-api.phoenix.trade/ws",
  });
    ...
    try {
      switch (channel) {
        ...
        case "candles": {
          ...
          console.log(
            `📡 Subscribing to: candles ${symbol} ${timeframe}`
          );
          for await (const update of wsClient.candles(symbol, timeframe, signal)) {
            console.log(`[candles:${symbol}:${timeframe}]`, JSON.stringify(update, jsonReplacer, 2));
          }
          break;
        }
        ...
        case "exchangeStatus":
          ...
          for await (const update of wsClient.exchangeStatus(signal)) {
            console.log(`[exchangeStatus]`, JSON.stringify(update, jsonReplacer, 2));
          }
          break;
        ...
        case "fills": {
          const symbol = args[0];
          console.log(
            `📡 Subscribing to: fills ${symbol || "(all)"}`
          );
        ...
        case "fundingRate": {
          if (args.length < 1) {
            console.error(`❌ fundingRate requires 1 argument: symbol (got ${args.length})`);
            return;
          }
          const symbol = args[0];
          console.log(`📡 Subscribing to: fundingRate ${symbol}`);
          for await (const update of wsClient.fundingRate(symbol, signal)) {
            console.log(`[fundingRate:${symbol}]`, JSON.stringify(update, jsonReplacer, 2));
          }
          break;
        }
        ...
        case "l2Book": {
          if (args.length < 1) {
            console.error(`❌ l2Book requires 1 argument: symbol (got ${args.length})`);
            return;
          }
          const symbol = args[0];
          console.log(`📡 Subscribing to: l2Book ${symbol}`);
          for await (const update of wsClient.l2Book(symbol, signal)) {
            console.log(`[l2Book:${symbol}]`, JSON.stringify(update, jsonReplacer, 2));
          }
          break;
        }
        ...
        case "markPrice": {
          if (args.length < 1) {
            console.error(`❌ markPrice requires 1 argument: symbol (got ${args.length})`);
            return;
          }
          const symbol = args[0];
          console.log(`📡 Subscribing to: markPrice ${symbol}`);
          for await (const update of wsClient.markPrice(symbol, signal)) {
            console.log(`[markPrice:${symbol}]`, JSON.stringify(update, jsonReplacer, 2));
          }
          break;
        }
        ...
        case "market": {
          if (args.length < 1) {
            console.error(`❌ market requires 1 argument: symbol (got ${args.length})`);
            return;
          }
          const symbol = args[0];
          console.log(`📡 Subscribing to: market ${symbol}`);
          for await (const update of wsClient.market(symbol, signal)) {
            console.log(`[market:${symbol}]`, JSON.stringify(update, jsonReplacer, 2));
          }
          break;
        }
        ...
        case "marketStats": {
          if (args.length < 1) {
            console.error(`❌ marketStats requires 1 argument: symbol (got ${args.length})`);
            return;
          }
          const symbol = args[0];
          console.log(`📡 Subscribing to: marketStats ${symbol}`);
          for await (const update of wsClient.marketStats(symbol, signal)) {
            console.log(`[marketStats:${symbol}]`, JSON.stringify(update, jsonReplacer, 2));
          }
          break;
        }
        ...
        case "orderbook": {
          if (args.length < 1) {
            console.error(`❌ orderbook requires 1 argument: symbol (got ${args.length})`);
            return;
          }
          const symbol = args[0];
          console.log(`📡 Subscribing to: orderbook ${symbol}`);
          for await (const update of wsClient.orderbook(symbol, signal)) {
            console.log(`[orderbook:${symbol}]`, JSON.stringify(update, jsonReplacer, 2));
          }
          break;
        }
        ...
        case "orderbookSnapshot": {
          if (args.length < 1) {
            console.error(`❌ orderbookSnapshot requires 1 argument: symbol (got ${args.length})`);
            return;
          }
          const symbol = args[0];
          console.log(`📡 Subscribing to: orderbookSnapshot ${symbol}`);
          for await (const update of wsClient.orderbookSnapshot(symbol, signal)) {
            console.log(`[orderbookSnapshot:${symbol}]`, JSON.stringify(update, jsonReplacer, 2));
          }
          break;
        }
        ...
        case "traderState": {
          ...
          console.log(`📡 Subscribing to: traderState ${authority} ${pdaIndex}`);
          for await (const update of wsClient.traderState(authority, pdaIndex, signal)) {
            console.log(
