...
export function PnlCalendar({ authority }: PnlCalendarProps) {
  ...
  function getCellColor(pnl: number | null, isPlaceholder: boolean): string {
    ...
    if (pnl === null) return "bg-[#1E293B] border border-[#334155]/50";
    if (pnl === 0) return "bg-[#334155] border border-[#334155]";
    const intensity = Math.min(Math.abs(pnl) / maxAbsPnl, 1);
    if (pnl > 0) {
      if (intensity > 0.75) return "bg-metric-buy border border-metric-buy/60";
      if (intensity > 0.5) return "bg-metric-buy/70 border border-metric-buy/40";
      if (intensity > 0.25) return "bg-metric-buy/40 border border-metric-buy/25";
      return "bg-metric-buy/20 border border-metric-buy/15";
    }
    if (intensity > 0.75) return "bg-metric-sell border border-metric-sell/60";
    if (intensity > 0.5) return "bg-metric-sell/70 border border-metric-sell/40";
    if (intensity > 0.25) return "bg-metric-sell/40 border border-metric-sell/25";
    return "bg-metric-sell/20 border border-metric-sell/15";
  }
  ...
  return (
    <div className="border border-metric-border bg-surface-1 p-4">
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1">
            ...
            <span className="h-2 w-2 rounded-[1px] bg-metric-sell/30 border border-metric-sell/20" />
            <span className="h-2 w-2 rounded-[1px] bg-metric-sell border border-metric-sell/60" />
            <span className="h-2 w-2 rounded-[1px] bg-[#334155] border border-[#334155]" />
            <span className="h-2 w-2 rounded-[1px] bg-metric-buy/30 border border-metric-buy/20" />
            <span className="h-2 w-2 rounded-[1px] bg-metric-buy border border-metric-buy/60" />
      ...
      {tooltip && (
        <div
          className="fixed z-50 -translate-x-1/2 px-2.5 py-1.5 border border-metric-border bg-surface-2 shadow-lg pointer-events-none"
        ...
        >
          ...
          <div
            className={clsx(
              ...
              tooltip.pnl > 0 ? "text-metric-buy" : tooltip.pnl < 0 ? "text-metric-sell" : "text-text-secondary/60"
            )}
          >
