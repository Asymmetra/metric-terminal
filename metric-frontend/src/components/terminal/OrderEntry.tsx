    ...
    <>
      <div className="flex h-full flex-col">
        <div className="flex flex-col gap-3 p-3">
          {/* Order type tabs */}
          <div className="flex border-b border-metric-border/50">
          ...
          {/* Side toggle */}
          {orderType !== "multi" && <><div className="grid grid-cols-2 gap-px bg-metric-border">
            <button
              onClick={() => setSide("buy")}
              className={clsx(
                "py-2 font-mono text-[11px] font-medium tracking-wider transition-all duration-150",
                side === "buy"
                  ? "bg-metric-buy text-metric-bg"
                  : "bg-surface-2 text-text-secondary/60 hover:text-text-secondary"
              )}
            >
              BUY / LONG
            </button>
          ...
          <div className={clsx("gap-px bg-metric-border", isIsolatedOnly ? "grid grid-cols-1" : "grid grid-cols-2")}>
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                step="0.01"
                className={clsx(
                  "w-full border bg-surface-2 py-2 pl-3 pr-12 font-mono text-[11px] text-text-primary placeholder:text-text-secondary/40 focus:outline-none transition-colors",
                  isolatedCollateralExceeded ? "border-metric-sell ring-1 ring-metric-sell/50 focus:border-metric-sell" :
                  needsCollateral ? "border-metric-primary/50 focus:border-metric-primary" : "border-metric-border focus:border-metric-primary/60"
                )}
              />
            ...
            <div className="mt-1 flex gap-1">
              {[10, 25, 50, 100].map((pct) => (
                <button
                  ...
                  className="flex-1 py-1 font-mono text-[9px] text-text-secondary/60 bg-surface-2 border border-metric-border/30 hover:text-metric-primary hover:border-metric-primary/40 transition-colors"
                >
                  {pct}%
                </button>
              ))}
            ...
            {isolatedCollateralExceeded && (
              <span className="mt-0.5 block text-[9px] text-metric-sell">
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className={clsx(
                  ...
                  needsPrice ? "text-metric-primary" : "text-text-secondary/70"
                )}>
                  Price {needsPrice && <span className="normal-case text-metric-primary/80">— required</span>}
                </label>
                {markPrice > 0 && (
                  <button
                    onClick={() => setPrice(markPrice.toFixed(2))}
                    className="font-mono text-[9px] text-metric-primary/70 hover:text-metric-primary transition-colors"
                  >
              ...
              <div className="relative">
                <input
                  ...
                  className={clsx(
                    "w-full border bg-surface-2 py-2 pl-3 pr-12 font-mono text-[11px] text-text-primary placeholder:text-text-secondary/40 focus:outline-none transition-colors",
                    needsPrice ? "border-metric-primary/50 focus:border-metric-primary" : "border-metric-border focus:border-metric-primary/60"
                  )}
                />
                <span className="absolute right-3 top-1/2 -translate-y-1/2 font-mono text-[10px] text-text-secondary/60">
                  USD
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  className="h-5 w-5 flex items-center justify-center border border-metric-border bg-surface-2 font-mono text-[11px] text-text-secondary hover:text-metric-primary hover:border-metric-primary/40 transition-colors"
                >
                  -
                </button>
                <span className="font-mono text-[11px] text-metric-primary min-w-[28px] text-center">{leverage}x</span>
                <button
            ...
            <input
              ...
              className="w-full accent-metric-primary"
            />
              ...
              {leverageBreakpoints.map((bp) => (
                <button
                  ...
                  className={clsx(
                    ...
                    leverage === bp ? "text-metric-primary" : "text-text-secondary/50 hover:text-text-secondary"
                  )}
                >
                  {bp}x
                </button>
          ...
          {orderType === "market" && <div>
            ...
            >
              ...
              <div className={clsx(
                ...
                showTpSl ? "bg-metric-primary" : "bg-metric-border"
              )}>
                <div className={clsx(
              ...
              <div className="mt-2 flex flex-col gap-2">
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    {tpPrice && markPrice > 0 && (
                      <span className="font-mono text-[10px] text-metric-buy">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className={clsx(
                        "w-full border bg-surface-2 py-2 pl-3 pr-12 font-mono text-[11px] text-text-primary placeholder:text-text-secondary/40 focus:outline-none transition-colors",
                        !tpValid ? "border-metric-sell focus:border-metric-sell" : "border-metric-border focus:border-metric-primary/60"
                      )}
                    />
                    <span className="absolute right-3 top-1/2 -translate-y-1/2 font-mono text-[10px] text-text-secondary/60">
                      USD
                  ...
                  <div className="mt-1 flex gap-1">
                    {[2.5, 5, 10].map((pct) => (
                      <button
                        ...
                        className="flex-1 py-1 font-mono text-[9px] text-text-secondary/60 bg-surface-2 border border-metric-border/30 hover:text-metric-buy hover:border-metric-buy/40 transition-colors"
                      >
                        +{pct}%
                      </button>
                    ))}
                  ...
                  {!tpValid && (
                    <span className="mt-0.5 text-[9px] text-metric-sell">
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className={clsx(
                        "w-full border bg-surface-2 py-2 pl-3 pr-12 font-mono text-[11px] text-text-primary placeholder:text-text-secondary/40 focus:outline-none transition-colors",
                        !slValid ? "border-metric-sell focus:border-metric-sell" : "border-metric-border focus:border-metric-primary/60"
                      )}
                    />
                    <span className="absolute right-3 top-1/2 -translate-y-1/2 font-mono text-[10px] text-text-secondary/60">
                      USD
                  ...
                  <div className="mt-1 flex gap-1">
                    {[2.5, 5, 10].map((pct) => (
                      <button
                        ...
                        className="flex-1 py-1 font-mono text-[9px] text-text-secondary/60 bg-surface-2 border border-metric-border/30 hover:text-metric-sell hover:border-metric-sell/40 transition-colors"
                      >
                        -{pct}%
                      </button>
                    ))}
          ...
          {/* Order summary */}
          {derivedOrder && (
            <div className="flex flex-col gap-1 border border-metric-border/30 bg-surface-2/30 p-2">
              ...
              {derivedOrder.liqPrice !== null && (
                <div className="flex justify-between">
                  ...
                  <span className="font-mono text-[10px] text-metric-sell">${formatPrice(derivedOrder.liqPrice)}</span>
                </div>
              )}
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-mono text-[10px] text-metric-primary">
              ...
              <div className="flex justify-between">
                ...
                <span className={clsx(
                  "font-mono text-[10px]",
                  marginMode === "isolated" ? "text-metric-primary" : "text-text-secondary"
                )}>
          ...
          {connected && activationState !== "active" && !fetchingAccount ? (
            activationState === "uninitialized" ? (
              <button
                ...
                className="flex w-full items-center justify-center py-2.5 font-mono text-[11px] font-medium tracking-wider bg-metric-primary text-white hover:brightness-110 active:brightness-95 transition-all duration-150"
              >
                DEPOSIT TO ACTIVATE
              </button>
            ) : (
              <button
                disabled
                className="flex w-full items-center justify-center py-2.5 font-mono text-[11px] font-medium tracking-wider bg-surface-2 text-text-secondary/50 cursor-not-allowed"
              >
            ...
            <button
              ...
              className={clsx(
                ...
                !validation.ready
                  ...
                  : side === "buy"
                    ? "bg-metric-buy text-metric-bg hover:brightness-110 active:brightness-95"
                    : "bg-metric-sell text-white hover:brightness-110 active:brightness-95",
        ...
        {connected && (
          <div className="mt-auto border-t border-metric-border/50 p-3">
            {activationState !== "active" && !fetchingAccount ? (
              <div className="flex items-center gap-2 rounded border border-metric-primary/30 bg-metric-primary/10 p-2">
                <svg className="h-3.5 w-3.5 flex-shrink-0 text-metric-primary" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5">
                  <path d="M8 4v4M8 12h.01M15 8A7 7 0 1 1 1 8a7 7 0 0 1 14 0z" />
                </svg>
                <span className="text-[10px] leading-tight text-metric-primary">
                  {activationState === "uninitialized"
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="flex items-center justify-between">
                <span className="text-[10px] uppercase tracking-wider text-metric-primary/80">Available</span>
                <span className="font-mono text-[11px] text-metric-primary">{formatUsd(freeCollateral)}</span>
              </div>
              <div className="flex items-center justify-between">
              ...
              {collateral > 0 && (() => {
                ...
                const barColor = marginPct > 80 ? "bg-metric-sell" : marginPct > 50 ? "bg-metric-primary" : "bg-metric-buy";
                return (
                  <div className="flex flex-col gap-1">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={clsx(
                        "font-mono text-[11px]",
                        marginPct > 80 ? "text-metric-sell" : marginPct > 50 ? "text-metric-primary" : "text-metric-buy"
                      )}>
                        {marginPct.toFixed(1)}%
                      </span>
                    </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className={clsx(
                  "font-mono text-[11px]",
                  unrealizedPnl >= 0 ? "text-metric-buy" : "text-metric-sell"
                )}>
