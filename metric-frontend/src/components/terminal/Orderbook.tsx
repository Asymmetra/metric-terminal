  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      {!snapshot ? (
        ...
        <>
          <div className="grid shrink-0 grid-cols-3 border-b border-metric-border/40 px-3 py-1 font-mono text-[9px] uppercase text-text-secondary/40">
            <span>Price</span>
            <span className="text-right">Size</span>
            <span className="text-right">Total</span>
          </div>
          {/* Fit any height: asks anchored just above the mid (extra rows clip off
              the top), bids just below (extra clip off the bottom). */}
          <div className="flex min-h-0 flex-1 flex-col">
            ...
            <div className="shrink-0 border-y border-metric-border/50 px-3 py-1 text-center font-mono text-[12px] text-text-primary">
...
function DepthRow({ r, side }: { r: { level: DepthLevel; total: number; pct: number }; side: "ask" | "bid" }) {
  ...
  return (
    <div className="relative grid shrink-0 grid-cols-3 px-3 py-0.5 font-mono text-[11px]">
