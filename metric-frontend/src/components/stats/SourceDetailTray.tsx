        ...
        <>
          ...
          */}
          <motion.div
            ...
            className="fixed right-0 top-0 bottom-0 z-[91] w-[580px] border-l border-metric-border bg-surface-1 shadow-[−24px_0_96px_rgba(0,0,0,0.55)] overflow-y-auto"
          >
            <Header
              source={source}
              onClose={onClose}
...
function Header({
  ...
  return (
    <div className="border-b border-metric-border/70 px-5 py-3">
      <div className="flex items-start justify-between gap-3">
        <div className="flex flex-col gap-1">
          <span className="font-mono text-[10px] uppercase tracking-wider text-metric-primary">{eyebrowText}</span>
        ...
        return (
          <button
            ...
            className={clsx(
              ...
              active
                ? "border-metric-primary/60 bg-metric-primary/15 text-metric-primary"
                : "border-metric-border text-text-secondary/70 hover:bg-surface-2 hover:text-text-primary",
            )}
...
function Body({ source, defaultLanguage, onLanguageChange }: { source: DataSource; defaultLanguage?: string; onLanguageChange?: (lang: string) => void }) {
  ...
  return (
    <div className="flex flex-col gap-4 p-5">
      ...
      {/* Tabs */}
      <div className="flex border border-metric-border bg-surface-2/40">
        {(["code", "payload", "history"] as const).map((k) => (
          <button
            key={k}
            onClick={() => setTab(k)}
            className={clsx(
              "flex-1 px-3 py-1.5 font-mono text-[10px] uppercase tracking-wider transition-colors border-r border-metric-border/40 last:border-r-0",
              tab === k ? "bg-metric-primary/10 text-metric-primary" : "text-text-secondary/60 hover:text-text-primary",
            )}
          >
            {k}
          </button>
      ...
      {tab === "payload" && (
        <div className="border border-metric-border bg-surface-2/40">
          <div className="border-b border-metric-border/50 px-3 py-1.5 font-mono text-[10px] uppercase tracking-wider text-text-secondary/60">Latest payload</div>
      ...
      {tab === "history" && (
        <div className="border border-metric-border bg-surface-2/40 max-h-[60vh] overflow-y-auto">
          <div className="border-b border-metric-border/50 px-3 py-1.5 font-mono text-[10px] uppercase tracking-wider text-text-secondary/60">Recent {source.recentPayloads.length} payloads (newest first)</div>
          <div className="flex flex-col">
            {[...source.recentPayloads].reverse().map((entry, i) => (
              <div key={i} className="border-b border-metric-border/20 px-3 py-1.5 font-mono text-[9px] text-text-secondary/70">
...
function MarketSnapshot({ payload }: { payload: PhoenixMarketPayload | null }) {
  ...
  return (
    <div className="border border-metric-border bg-surface-2/40">
      <div className="border-b border-metric-border/50 px-3 py-1.5">
        <span className="font-mono text-[10px] uppercase tracking-wider text-text-secondary/60">Market snapshot</span>
      </div>
      <div className="grid grid-cols-3 gap-px bg-metric-border/40">
        <PriceCell
          label="Oracle"
          value={oracle}
      ...
      <div className="grid grid-cols-4 gap-px bg-metric-border/40">
        ...
        <MetricCell label="Funding rate"   value={`${fundingPct.toFixed(4)}%`}      tooltip="Current funding rate. Positive = longs pay shorts (mark > oracle persistently); negative = shorts pay longs. Settled every 8h on a rolling epoch." colored={fundingPct >= 0 ? "text-metric-buy" : "text-metric-sell"} />
        <MetricCell label="24h change"     value={`${change24h >= 0 ? "+" : ""}${change24h.toFixed(2)}%`} tooltip="Mark price change vs the mark price 24 hours ago." colored={change24h >= 0 ? "text-metric-buy" : "text-metric-sell"} />
      </div>
    </div>
  );
}
...
function HoverHint({ text }: { text: string }) {
  return (
    <span
      role="tooltip"
      className="pointer-events-none invisible absolute left-2 top-full z-50 mt-1 whitespace-normal rounded border border-metric-border bg-surface-l3/95 px-2 py-1.5 font-mono text-[10px] leading-snug text-text-primary opacity-0 shadow-xl backdrop-blur-sm transition-opacity duration-75 group-hover:visible group-hover:opacity-100"
...
function PriceCell({ label, value, color, tooltip }: { label: string; value: number; color: string; tooltip: string }) {
  return (
    <div className="group relative flex flex-col gap-0.5 bg-surface-1 px-3 py-2.5">
...
function SpreadCell({ label, deltaUsd, deltaBps, tooltip }: { label: string; deltaUsd: number; deltaBps: number; tooltip: string }) {
  ...
  const color = Math.abs(deltaBps) < 1 ? "text-text-secondary/60" : isPos ? "text-metric-buy" : "text-metric-sell";
  return (
    <div className="group relative flex flex-col gap-0.5 bg-surface-1 px-3 py-2">
      <span className="cursor-help font-mono text-[9px] uppercase tracking-wider text-text-secondary/50 border-b border-dotted border-text-secondary/30 w-fit">{label}</span>
...
function CadencePanel({ source }: { source: DataSource }) {
  ...
  return (
    <div className="border border-metric-border bg-surface-2/40">
      ...
      <div className="border-t border-metric-border/50 px-3 py-2">
...
function StatBox({ label, value, tooltip, highlight }: { label: string; value: string; tooltip?: string; highlight?: boolean }) {
  return (
    <div className="group relative flex flex-col gap-0.5 bg-surface-1 px-2 py-1.5">
      ...
      <span className={clsx("font-mono text-xs", highlight ? "text-metric-primary" : "text-text-primary")}>{value}</span>
...
function statusBadge(s: DataSource["status"]) {
  switch (s) {
    case "healthy":  return { label: "Healthy",  cls: "text-metric-buy",       dot: "bg-metric-buy" };
    case "degraded": return { label: "Degraded", cls: "text-yellow-500",        dot: "bg-yellow-500" };
    case "stale":    return { label: "Stale",    cls: "text-metric-sell",         dot: "bg-metric-sell" };
    case "error":    return { label: "Error",    cls: "text-metric-sell",         dot: "bg-metric-sell" };
