...
export default function StatsPage() {
  ...
  useEffect(() => {
    ...
    setExpanded({
      ...
      "metric-ws":     prefs.expandedCategories["metric-ws"]     ?? true,
      "metric-rest":   prefs.expandedCategories["metric-rest"]   ?? true,
    });
  }, []);

  // Pull market list (also auto-refreshed by the hook every 5 min via the
  // metric-rest-markets poller; this initial fetch just seeds subscriptions
  // immediately).
  useEffect(() => {
    let cancelled = false;
    const refresh = async () => {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-metric-bg text-text-primary">
      {/* Nav header */}
      <div className="flex items-center justify-between border-b border-metric-border bg-surface-1 px-4 py-2.5">
        <div className="flex items-center gap-3">
          <Link href="/" className="font-mono text-xs font-medium uppercase tracking-wider text-metric-primary hover:text-metric-primary/80">Ember</Link>
        ...
        <div className="flex items-center gap-3">
          <Link href="/terminal" className="font-mono text-[10px] uppercase tracking-wider text-text-secondary/60 hover:text-metric-primary transition-colors">Terminal</Link>
          <Link href="/profile" className="font-mono text-[10px] uppercase tracking-wider text-text-secondary/60 hover:text-metric-primary transition-colors">Profile</Link>
          <Link href="/leaderboard" className="font-mono text-[10px] uppercase tracking-wider text-text-secondary/60 hover:text-metric-primary transition-colors">Leaderboard</Link>
          <WalletButton />
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2 px-4 pt-4">
        ...
        {marketsError && (
          <div className="border border-metric-sell/40 bg-metric-sell/10 px-3 py-2 font-mono text-[10px] text-metric-sell">
      ...
      <div className="flex flex-wrap items-center gap-1.5 border-b border-metric-border/40 bg-surface-1/60 px-4 py-2 mt-3">
          ...
          return (
            <button
              ...
              className={clsx(
                ...
                isVisible
                  ? "border-metric-primary/40 bg-metric-primary/10 text-metric-primary"
                  : "border-metric-border text-text-secondary/50 hover:text-text-primary",
              )}
            >
      ...
      <div className="flex flex-wrap items-center gap-3 border-b border-metric-border/40 bg-surface-1 px-4 py-3">
        ...
        {snapshot.categoryHealth["metric-ws"].total > 0 &&     <CategoryDot cat="metric-ws"     stats={snapshot.categoryHealth["metric-ws"]}     label="Ember WS" />}
        {snapshot.categoryHealth["metric-rest"].total > 0 &&   <CategoryDot cat="metric-rest"   stats={snapshot.categoryHealth["metric-rest"]}   label="Ember REST" />}
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          <input
            ...
            className="border border-metric-border bg-metric-bg/60 px-2 py-1 font-mono text-[10px] text-text-primary outline-none focus:border-metric-primary/60 w-40"
          />
          ...
          <button
            onClick={resetAll}
            className="border border-metric-border px-2 py-1 font-mono text-[10px] uppercase tracking-wider text-text-secondary/70 hover:bg-surface-2 hover:text-text-primary transition-colors"
      ...
      {snapshot.phoenixWs.state === "error" && (
        <div className="border-b border-metric-sell/40 bg-metric-sell/10 px-4 py-2 font-mono text-[10px] text-metric-sell">
...
function CategoryDot({ cat, stats, label }: { cat: SourceCategory; stats: { healthy: number; degraded: number; stale: number; error: number; idle: number; total: number }; label: string }) {
  ...
  const dotColor = stats.total === 0 ? "bg-text-secondary/30"
    : anyBad ? "bg-metric-sell"
    : stats.degraded > 0 ? "bg-yellow-500"
    : allHealthy ? "bg-metric-buy"
